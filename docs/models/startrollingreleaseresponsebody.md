...
```typescript
import { StartRollingReleaseResponseBody } from "@vercel/sdk/models/startrollingreleaseop.js";

let value: StartRollingReleaseResponseBody = {
  rollingRelease: {
    state: "ACTIVE",
    substate: "PAUSED",
    currentDeployment: {
...
| -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `rollingRelease`                                                                                               | [models.StartRollingReleaseRollingRelease](../models/startrollingreleaserollingrelease.md)                     | :heavy_check_mark:                                                                                             | Rolling release information including configuration and document details, or null if no rolling release exists |