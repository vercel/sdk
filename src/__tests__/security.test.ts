...
it.skip("Security Get /V1 /Security/firewall/events", async () => {
  assert.fail(
    "incomplete test found please make sure to address the following errors: [`workflow step get_/v1/security/firewall/events.test referencing operation get_/v1/security/firewall/events not found in document`]",
  );
});

test("Security Get Security Firewall Events", async () => {
  const testHttpClient = createTestHTTPClient("getSecurityFirewallEvents");
