...
import {
  ...
  GetConnectorTokenRequest$outboundSchema,
  GetConnectorTokenResponseBody,
  GetConnectorTokenResponseBody$inboundSchema,
 ...
 */
export function connectGetConnectorToken(
  client: VercelCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => GetConnectorTokenRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/connect/token/{connector}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, GetConnectorTokenResponseBody$inboundSchema),
    M.fail([400, 401, 403, 404, 422, 429, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
