...
import {
  ...
  GenerateFirewallRuleRequest$outboundSchema,
  GenerateFirewallRuleResponseBody,
  GenerateFirewallRuleResponseBody$inboundSchema,
 ...
 */
export function firewallGenerateFirewallRule(
  client: VercelCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => GenerateFirewallRuleRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/security/firewall/config/generate-rule")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, GenerateFirewallRuleResponseBody$inboundSchema),
    M.fail([400, 401, 403, 404, 408, 410, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
