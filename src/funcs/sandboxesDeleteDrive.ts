...
import {
  DeleteDriveRequest,
  DeleteDriveRequest$outboundSchema,
  DeleteDriveResponseBody,
  DeleteDriveResponseBody$inboundSchema,
 ...
 */
export function sandboxesDeleteDrive(
  client: VercelCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => DeleteDriveRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/sandboxes/drives/{name}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, DeleteDriveResponseBody$inboundSchema),
