...
import {
  ListDrivesRequest,
  ListDrivesRequest$outboundSchema,
  ListDrivesResponseBody,
  ListDrivesResponseBody$inboundSchema,
 ...
 */
export function sandboxesListDrives(
  client: VercelCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => ListDrivesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v2/sandboxes/drives")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, ListDrivesResponseBody$inboundSchema),
    M.fail([400, 401, 402, 403, 404, 429, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
