...
export type OneHundredAndThirtySeven = {
  edgeConfigId: string;
  edgeConfigSlug: string;
  globalConfigSchema?: GlobalConfigSchema | undefined;
};
...
/** @internal */
export const GlobalConfigSchema$inboundSchema: z.ZodType<
  GlobalConfigSchema,
  z.ZodTypeDef,
  unknown
> = z.object({});

export function globalConfigSchemaFromJSON(
  jsonString: string,
): SafeParseResult<GlobalConfigSchema, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => GlobalConfigSchema$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'GlobalConfigSchema' from JSON`,
  );
}
...
> = z.object({
  edgeConfigId: types.string(),
  edgeConfigSlug: types.string(),
  globalConfigSchema: types.optional(
    z.lazy(() => GlobalConfigSchema$inboundSchema),
  ),
});
